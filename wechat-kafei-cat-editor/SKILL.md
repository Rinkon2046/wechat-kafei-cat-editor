...
- 所有普通正文段落使用较克制的小字号，建议 `font-size:15px;line-height:1.75;`，并首行缩进两个中文字符，可用内联 CSS 如 `text-indent:2em;`。仅应用于正文段落；不要缩进标题、副标题/导语、引用块、列表行、图片说明、表格、引用区或 UI 式标签。
- 将关键结论渲染为左边框强调块。
...
- `>` 引用块会成为左边框强调块。
- `##` 标题会成为编号章节。
- 普通段落会成为小字号、带两个中文字符首行缩进的微信公众号正文段落，例如 `font-size:15px;line-height:1.75;text-indent:2em;`。
